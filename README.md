...
一个基于 `Qt Widgets` 的图形化联调工程，用来给 [`gdbus-learn`](https://github.com/SweerItTer/gdbus-learn) 提供可视化客户端，替代原来的命令行交互方式。当前版本只对接外部已启动的 `system bus` 服务端，不再内置本地 demo service，也不再使用临时 `session bus`。
...
- 使用现有文件发送接口进行文件选择与发送
- 使用新增下载接口把服务端文件下载到本地
- 在界面上明确显示 `system bus` 连接状态和文件操作结果
...
- 通过 `DownloadFile` 下载文件
- 单独显示文件操作成功 / 失败结果
- 一键 `Release` 构建并启动 GUI
...
- `scripts/run_release.sh`
  一键 `Release` 构建并启动 GUI，本身不负责启动服务端。
...
- `Select File`
  打开文件选择框，记录当前待发送文件路径。
- `Send File`
  调用 `TrainingClient::SendFileByPath(...)` 发送文件，可选填写远端相对路径。
- `Download File`
  调用 `TrainingClient::DownloadFile(...)`，按“远端相对路径 -> 本地目标路径”执行下载。
- 文件操作结果标签
  明确显示上传或下载成功 / 失败，而不是只放在普通状态栏里。
...
GUI 只负责连接，运行前必须先把后端按 `system bus` 方式配置好。

你需要先在 `gdbus-learn` 仓库完成这些步骤：

1. 构建后端
2. 安装 D-Bus policy 和 systemd unit
3. 启动真实 `system bus` 服务端
...
1. 点击 `Select File`
2. 选择一个本地文件
3. 可选填写 `Upload remote path`
4. 点击 `Send File`
5. 观察界面中的文件操作结果标签

### 文件下载测试

1. 在后端服务端目录中准备一个可下载文件
2. 在 `Download remote path` 中填写远端相对路径
3. 在 `Download target` 中填写本地保存路径，或点 `Save As`
4. 点击 `Download File`
5. 观察界面中的文件操作结果标签
6. 检查本地目标文件内容
...
- 远端路径是相对于后端服务文件根目录的路径。
- GUI 不负责启动服务端，只连接已经运行在 `system bus` 上的 `com.example.Training`。
- 如果启动时看到 system bus 连接错误，优先检查后端的 policy、service 和 `training.service` 状态。
3. 点击 `Send File`
4. 观察界面中的文件发送结果标签
